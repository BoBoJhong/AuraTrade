...
interface Props {
    ...
    onToggleExpand: (symbol: string | null) => void
...
export const WatchlistStockCard = ({
    item,
    index,
    expandedStock,
    onToggleExpand,
    ...
    // WebSocket 實時價格更新 (在初始數據載入後啟動)
    const { priceData: wsData, isConnected } = useStockWebSocket(stock.symbol, true)

    // 使用 WebSocket 數據或初始數據
    const currentPrice = wsData?.price ?? initialPrice
    const change = wsData?.change ?? initialChange
    const changePercent = wsData?.change_percent ?? initialChangePercent

    // Debug: 顯示價格來源
    console.log(`💰 ${stock.symbol} 價格:`, {
        source: wsData ? 'WebSocket' : 'Initial',
        currentPrice,
        initialPrice,
        wsPrice: wsData?.price,
        isConnected
    })
    ...
    return (
        <div
            className="glass rounded-2xl shadow-lg hover:shadow-xl animate-fadeIn transition-all duration-300 relative"
            style={{ animationDelay: `${index * 0.1}s` }}
        >
            {/* WebSocket 連接狀態指示器 */}
            {isConnected && (
                <div className="absolute top-2 right-2 z-20">
                    <div className="flex items-center gap-1 text-xs text-green-400 bg-green-500/10 px-2 py-1 rounded-full border border-green-500/30">
            ...
            <div
                ...
                onClick={() => onToggleExpand(isExpanded ? null : stock.symbol)}
            >
                {/* Gradient Background */}
